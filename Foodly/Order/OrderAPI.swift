...
extension OrderAPI: FirestoreRequest {
    ...
    var documentReference: DocumentReference? {
        ...
        case .getFoodOrderHistory:
            return baseCollectionReference?.collection(Collection.orderHistory.identifier).document("id")
        case .updateFoodOrder( let orderId, _):
            return baseCollectionReference?.collection(Collection.orderHistory.identifier).document(orderId)
        }
    }
